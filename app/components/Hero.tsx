...
import { BookOpen, Brain, Zap, LogOut, Globe, TrendingUp, Menu, X } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-study-bg to-primary/5">
      <div className="container mx-auto px-4 py-16">
        {/* Navigation */}
        <nav className="flex items-center justify-between mb-16 relative">
          ...
          {isMobileMenuOpen && (
            <div className="absolute top-full left-0 right-0 bg-background border border-border rounded-lg shadow-lg mt-2 py-2 z-50 md:hidden">
              {isAuthenticated ? (
                <>
                  <div className="flex items-center gap-2 px-4 py-2 text-sm text-green-800">
                    <div className="w-2 h-2 bg-green-600 rounded-full"></div>
                    Signed In
                  </div>
                  ...
                  <hr className="my-2 border-border" />
                  <button 
                    className="w-full text-left px-4 py-2 hover:bg-muted font-medium text-primary"
                    onClick={() => {
                      router.push('/study');
                      setIsMobileMenuOpen(false);
                    }}
